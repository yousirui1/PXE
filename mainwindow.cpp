    ...
    ui(new Ui::MainWindow)
{
    ...
    memset(&dhcp_config, 0, sizeof(dhcp_param));
...
void MainWindow::addItem(int index, char *ip, char *mac)
{
    ...
    item->ip_label->setText(QString(ip));
    item->mac_label->setText(QString(mac));
...
void MainWindow::onUpdateTime()
{
    ...
    {
        if(global->clients && global->clients[i])
        {
            sprintf(ip_str, "%d.%d.%d.%d", u_ip[0], u_ip[1], u_ip[2], i + u_ip[3]);
            addItem(i, ip_str, ip_str);
        }
    }
}
...
void MainWindow::modifiyConfig()
{
    QString strAll;
    QStringList strList;
    QFile readFile(".//voi_pxe//boot.ipxe");
    ...
    readFile.close();
    QFile writeFile(".//voi_pxe//boot.ipxe");
    if(writeFile.open(QIODevice::WriteOnly|QIODevice::Text))
    {
        ...
        for(int i=0;i<strList.count();i++)
        {
            ...
            if(strList.at(i).contains("dhcp"))
            {
                ...
                tempStr.replace(0,tempStr.length(), QString("kernel http://%1/boot/vmlinuz-5.2.8-lfs-9.0 server_ip=%2 tftp_ip=%3 http_ip=%4 ro vga=0x317 ")
                                .arg(QString(dhcp_config.local_ip)).arg(ui->server_edit->text()).arg(QString(dhcp_config.local_ip)).arg(QString(dhcp_config.local_ip)));
                stream<<tempStr<<'\n';
                i+=1;
            }
        }
...
void MainWindow::on_run_btn_clicked()
{
	...
	strcpy(dhcp_config.tftp_ip, dhcp_config.local_ip);
    dhcp_config.lease = ui->lease_edit->text().toInt();

    unsigned  int ip = inet_addr(dhcp_config.pool_ip);
    ...
    else
    {
         ...
         modifiyConfig();
         Global *global = Global::getGlobal();
         global->clients = get_clients(&dhcp_config);
         start_dhcpd();
    }
    run_flag = !run_flag;
}
